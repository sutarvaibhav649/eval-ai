 ...
 * 1. Subject creation
 * 2. Exam creation
 * 3. Batch PDF upload + directory structure creation
...
public class AdminService {
     ...
     * Throws if subject code already exists — codes must be unique across the system.
     *
     ...
     * Throws if subject not found or duplicate exam title exists for same subject.
     *
     ...
     *
     * For each PDF:
     * 1. Creates directory structure on disk
     * 2. Saves PDF to upload/uploaded_pdf/examId/studentId/
     * 3. Splits PDF into individual page images → upload/raw_images/examId/studentId/
     * 4. Creates AnswerSheet record in DB (status: PENDING)
     * 5. Creates TaskLog record (status: QUEUED)
     *
     ...
     * 				PRIVATE HELPER METHODS
     ---------------------------------------------------*/
	/**
     * Builds a file system path from parts.
     * Example: buildPath("upload", "uploaded_pdf", "exam1", "student1")
    ...
    /**
     * Splits a PDF file into individual PNG images, one per page.
     * Uses Apache PDFBox.
     *
     * Writes files as: page_1.png, page_2.png, page_3.png ...
